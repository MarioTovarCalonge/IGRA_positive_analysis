...
for(mode_gen in modes){
    output_dir <- paste0("Outputs/Model_", sprintf("%d", mode_gen), "/VE_dis_dist/")
    ...
    plot1 <- ggplot() +
            geom_ribbon(aes(x = df$eps, ymin = df$y_min, ymax = df$y_max), fill = "grey70") +
            geom_line(aes(x = df$eps, y = df$yval)) +
            geom_hline(aes(yintercept=49.7), color='black', linetype="dashed") + 
            labs(x = expression(epsilon), y = "VEdis(%)" ) +
            theme(axis.line = element_line(colour = "black", size=0.3), legend.title=element_blank() )
                
    if(mode_gen!=3){
        plot1 <- plot1 + geom_errorbarh(aes(y=49.7, xmin = eps_low, xmax = eps_hi), height=6, color="blue") + geom_point(aes(x=eps_med, y=49.7), color='blue')
    } 
                
    ggsave(paste0('Outputs/Model_', sprintf("%d", mode_gen), '/Cloud_Model_', sprintf("%d", mode_gen), '_A_', age_gen, '.png'), width = 10, height = 7, plot1)
...
p4 <- ggplot() + 
        geom_point(data = dfL, aes(x=Model, y = LM, color = 'Likelihood')) + 
        geom_errorbar(data = dfL, aes(x = Model, ymin = LM_low, ymax= LM_up), width=.2, color='blue') +
        labs(x = "Model", y = 'L(M|VEdis=49.7)' ) +
        scale_colour_manual(values = c('Likelihood' = 'blue')) +
        theme(axis.line = element_line(colour = "black", size=0.3), legend.title=element_blank() ) + 
        scale_x_continuous(breaks = seq(1, 7, by = 1))

ggsave(paste0("Outputs/Likelihood_age_", age_gen, ".png"), width = 10, height = 7, p4)
